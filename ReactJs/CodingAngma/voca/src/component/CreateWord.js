...
export default function CreateWord() {
    ...
    const [isLoading, setIsLoading] = useState(false); // isLoading이 false일 때만 함수를 실행하게 해준다.

    function onSubmit(e) { // onSubmit 함수 생성해서 <form>에 연결해준다.
        // 기본기능 막아주기
        e.preventDefault(); // 버튼을 눌러도 새로고침 되지 않는다.

        // 통신중 버튼을 어려번 클릭할 때 반응하지 않게 하기
        if(!isLoading) { // isLoading이 false이면 실행
            setIsLoading(true); // true로 바꿔준다.
            ...
            fetch(`http://localhost:3001/words/`, { // words 까지만 입력
                ...
                if(res.ok) { // 응답이 ok면 
                    ...
                    setIsLoading(false); // 모든 작업이 완료되면 false로 바꿔준다.
                }
            });
        }
    }
    ...
    <form onSubmit={onSubmit}>
        ...
        {/* isLoading 상태일 때는 Saving...이라고 표현해준다.  */}
        <button
            style={{
        ...
        >
            {isLoading ? "Saving..." : "저장"}
        </button> {/* 저장 버튼을 클릭하는 시점은 랜더링 결과가 DOM에 반영 된 이후. */}
    </form>
    );
}